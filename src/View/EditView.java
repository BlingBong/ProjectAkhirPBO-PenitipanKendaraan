...
public class EditView extends JFrame implements ActionListener {
    ...
    public EditView(String[] cek, String[] editData) {
        setTitle("Edit " + editData[1]);
        ...
        indeks = editData[0];
        etPlat.setText(editData[1]);
        compareRB(jkGroup, editData[2]);
        compareRB(tGroup, editData[3]);
        tglMasuk = editData[4];
    }
            ...
            if (etPlat.getText().isEmpty()) {
                ...
                try {
                    ...
                    if (plat.length() > 10){
                        ...
                        String[] dataTitip = {
                                indeks, plat, jenis, nomor, tglMasuk
                        };

                        MainController titip = new MainController();
                        titip.editExecute(dataTitip);
                        dispose();
                    }
                }
    ...
    public void compareRB(ButtonGroup buttonGroup, String existRB) {
        for (Enumeration<AbstractButton> bg = buttonGroup.getElements(); bg.hasMoreElements();) {
            AbstractButton rb = bg.nextElement();

            if (rb.getText().equals(existRB)) {
                rb.setSelected(true);
            }
        }
    }
